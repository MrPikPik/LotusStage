...
namespace AmplifyShaderEditor
{
	...
	public class PropertyNode : ParentNode
	{
		...
		private const int LongNameSize = 200;
		...
		public override void Draw( DrawInfo drawInfo )
		{
			...
			if( ContainerGraph.LodLevel <= ParentGraph.NodeLOD.LOD3 )
			{
				...
				if( m_isEditing || m_startEditing )
				{
					...
					if( EditorGUI.EndChangeCheck() )
					{
						SetClippedTitle( m_propertyInspectorName, LongNameSize );
						m_sizeIsDirty = true;
						m_isDirty = true;
						if( m_propertyInspectorName.Length > 0 )
						{
		...
		public override void OnNodeLayout( DrawInfo drawInfo )
		{
			...
			if( m_currentParameterType != m_lastParameterType || m_propertyNameIsDirty )
			{
				...
				if( m_currentParameterType != PropertyType.Constant )
				{
					...
					SetClippedAdditionalTitle( string.Format( m_useVarSubtitle ? Constants.SubTitleVarNameFormatStr : Constants.SubTitleValueFormatStr, currValue ), LongNameSize, LongNameEnder );
					//}
					//else
					//{
				...
				{
					SetClippedTitle( m_propertyInspectorName, LongNameSize );
					SetClippedAdditionalTitle( string.Format( Constants.SubTitleConstFormatStr, GetPropertyValStr() ), LongNameSize, LongNameEnder );
				}
			}
